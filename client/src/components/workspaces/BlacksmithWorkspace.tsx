...
const BlacksmithWorkspace = () => {
    ...
    const enchantConfig = CITY_ENCHANT_CONFIGS['ferrum'];
    ...
    const openRequirementModalFromResult = (result: WorkspaceActionResult) => {
        ...
        if (!requirement?.requiredItemName) {
            setRequirementModal({
                open: true,
                title: t('workspace.cannot_start_smelt'),
    ...
    const submitCookSelection = async () => {
        ...
        // using startCook for smelting currently
    ...
    return (
        <>
            <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                <UtensilsCrossed style={{ width: '0.7rem', height: '0.7rem' }} /> {t('workspace.smelter', { label: secondaryJobLabel })}
            </div>
                        ...
                        >
                            ...
                            >
                                <div style={{ fontSize: '0.85rem', fontWeight: 700, color: 'rgba(255,255,255,0.92)' }}>
                                    {t('workspace.choose_ingredients_smelt', { name: selectedRecipe.name })}
                                </div>
            ...
            {(() => {
                ...
                const materialCounts = enchantConfig.materialChain.map((t) => ({
                    name: t.itemName,
                    qty: inventory.filter((s) => s.item?.name === t.itemName).reduce((sum, s) => sum + s.quantity, 0),
                }));

                return enchantableSlots.length > 0 ? (
                    <div style={{ marginTop: '0.75rem', borderTop: '1px solid rgba(251,146,60,0.15)', paddingTop: '0.6rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.45rem', fontSize: '0.68rem', color: '#fb923c', fontWeight: 700 }}>
                            <Flame style={{ width: '0.7rem', height: '0.7rem' }} />
                            Upgrade Forge
                        </div>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem', marginBottom: '0.4rem' }}>
                            {materialCounts.map((m) => (
                                <span key={m.name} style={{ fontSize: '0.6rem', background: 'rgba(251,146,60,0.12)', border: '1px solid rgba(251,146,60,0.25)', borderRadius: '4px', padding: '2px 6px', color: '#fdba74' }}>
                                    {m.name}: {m.qty}
                                </span>
                            ))}
                        </div>
                        {enchantableSlots.map((s) => {
                            ...
                            return (
                                <div key={s.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.35rem 0.5rem', marginBottom: '0.3rem', borderRadius: '0.4rem', border: '1px solid rgba(251,146,60,0.15)', background: 'rgba(251,146,60,0.04)' }}>
                                    ...
                                    <motion.button
                                        ...
                                        style={{ fontSize: '0.6rem', fontWeight: 700, padding: '0.2rem 0.5rem', borderRadius: '4px', border: '1px solid #fb923c', background: 'rgba(251,146,60,0.15)', color: '#fb923c', cursor: 'pointer' }}
                                    >
                                        🔥 Forge Up
                                    </motion.button>
                                </div>
...
export default BlacksmithWorkspace;
