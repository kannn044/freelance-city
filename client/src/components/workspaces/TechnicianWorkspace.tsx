...
const TechnicianWorkspace = () => {
    ...
    const handleStartExtract = async (itemId: number) => {
        const itemOrders = workOrders.filter(
            (o) => (o.type === 'EXTRACT' || o.type === 'FARM') && o.item_id === itemId && !o.collected
        );
    ...
    return (
        <>
            ...
            {seedSlots.length === 0 ? (
                ...
                seedSlots.map((slot) => {
                    const itemOrders = slot.item ? workOrders.filter((o) => (o.type === 'EXTRACT' || o.type === 'FARM') && o.item_id === slot.item!.id && !o.collected) : [];
                    ...
                    return (
                        <motion.button
                            ...
                            style={{
                                ...
                                borderRadius: '0.5rem',
                                border: `1px solid ${isReadyBlocked ? 'rgba(251,191,36,0.55)' : isPlotFull ? 'rgba(103,232,249,0.5)' : 'rgba(255,255,255,0.12)'}`,
                                background: isReadyBlocked ? 'rgba(251,191,36,0.07)' : isPlotFull ? 'rgba(103,232,249,0.07)' : 'rgba(255,255,255,0.03)',
                        ...
                        >
                            ...
                            <span style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.15rem' }}>
                                <span style={{ fontSize: '0.65rem', color: isReadyBlocked ? '#fbbf24' : 'rgba(103,232,249,0.7)', fontWeight: 600 }}>
                                    {t('workspace.extract', { defaultValue: 'Extract' })}
                                </span>
                                <span style={{ fontSize: '0.58rem', color: isReadyBlocked ? '#fbbf24' : isPlotFull ? '#67e8f9' : 'rgba(255,255,255,0.35)', fontWeight: isReadyBlocked || isPlotFull ? 700 : 400 }}>
