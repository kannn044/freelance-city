...
const MarketplacePage = () => {
    ...
    const recipeShop = useGameStore((s) => s.recipeShop);
    // Actions are stable refs in Zustand — won't cause re-renders
    const fetchMarket = useGameStore((s) => s.fetchMarket);
    const fetchInventory = useGameStore((s) => s.fetchInventory);
    const fetchSalesHistory = useGameStore((s) => s.fetchSalesHistory);
    const fetchShop = useGameStore((s) => s.fetchShop);
    const fetchRecipeShop = useGameStore((s) => s.fetchRecipeShop);
    const buyListing = useGameStore((s) => s.buyListing);
    const buyFromShop = useGameStore((s) => s.buyFromShop);
    const buyRecipeUnlock = useGameStore((s) => s.buyRecipeUnlock);
    ...
    const listings = useMemo(() => {
        ...
        const filtered = marketListings
            ...
            .filter((l) => {
                // Affordable filter uses unit price only (not qty) to avoid marketBuyQty dependency
                if (!showAffordableOnly || !user) return true;
                return user.money >= l.price;
            });
        ...
        return sorted;
    }, [cityFilter, marketListings, maxPrice, minPrice, rarityFilter, search, showAffordableOnly, sortMode, typeFilter, user]);
...
const blobTopRightStyle: CSSProperties = {
    position: 'absolute',
    top: '-6rem',
    right: '-8rem',
    width: '30rem',
...
const blobBottomLeftStyle: CSSProperties = {
    position: 'absolute',
    bottom: '-9rem',
    left: '-10rem',
    width: '34rem',
